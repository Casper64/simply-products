<template>
  <div v-if="$route.params.slug !== 'logout'" class="login-page">
    <h1>Welcome</h1>
    <div class="forms">
       <Input v-if="!isLogin" v-model="username" @enter="submit" placeholder="username">
      ...
      <Input v-model="email" @enter="submit" placeholder="user@example.com">
      ...
      <Checkbox style="margin-top: 2em; font-size: 0.875rem"  v-model="remember">Remember me</Checkbox>
    ...
    <p class="wrong-side">
      ...
      <NuxtLink :to="isLogin ? '/auth/signup' : '/auth/login'">
  ...
  <div v-else class="login-page">
    <h1>Logging out...</h1>
    <p>Click <NuxtLink to="/">here</NuxtLink> if you are not redirected within 10 seconds</p>
  </div>
</template>
...
function validatePassword() {
  let valid = password.value.length > 7;
...
async function submit() {
  ...
  if (login) {
    ...
    try {
      ...
      let regex = new RegExp(/\d+\s*(.+)\s*\(/g);
      serverError.value = regex.exec(error.message)[1];
    }
  }
  // Singup
  else {
...
onMounted(() => {
  if (route.params.slug != 'logout') return
